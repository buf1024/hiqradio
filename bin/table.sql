...
create index `country_code_idx` on `state`(country, countrycode);
...
create index `language_code_idx` on `language`(language, languagecode);
...
create index `station_custom_idx` on `station_custom`(
    stationuuid,
    name,
    tags,
    country,
